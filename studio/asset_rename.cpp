...
AssetRenameDialog::AssetRenameDialog(Gtk::Window& parent, KageAsset *p_asset)
		: Gtk::Dialog("Rename Asset", parent, true),
		m_HBox(false, 8),
		_label("Label:", true) {
//	add_button(Gtk::Stock::OK, Gtk::RESPONSE_OK);
	...
	get_vbox()->pack_start(m_HBox, Gtk::PACK_SHRINK);
		...
		m_HBox.pack_start(_txtLabel, Gtk::PACK_SHRINK);
			_txtLabel.set_text(p_asset->getLabel());
			_asset = p_asset;
			_txtLabel.signal_activate().connect( sigc::mem_fun(*this, &AssetRenameDialog::EntryAssetLabel_onEnter) );
