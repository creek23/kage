...
void Kage::Delete_onClick() {
	if (KageStage::toolMode == KageStage::MODE_SELECT) {
		...
		if (m_KageStage.deleteSelectedShape() == true) {
			forceRenderFrames();
		}
...
void Kage::updateColors() {
	m_ColorButtonFill.set_color(m_KageStage.getFill());
	m_EntryFillAlpha.set_text(uintToString(KageStage::fillColor.getA()));
	m_ColorButtonStroke.set_color(m_KageStage.getStroke());
	m_EntryStrokeAlpha.set_text(uintToString(KageStage::stroke.getA()));
		...
		case Gtk::RESPONSE_OK:
			...
			saveKageStudio(ksfPath, "<stage width=\"" + uintToString(m_KageStage.stageWidth) + "\" height=\"" + uintToString(m_KageStage.stageHeight) + "\" " +
				...
				l_currentFrame = m_KageFramesManager.getCurrentFrame();
					for (i = 1; i <= l_lMax; i++) {
						m_KageFramesManager.setCurrentLayer(i);
						saveKageStudio(ksfPath, "<layer" + uintToString(i) + ">");
						for (j = 1; j <= l_fMax; ++j) {
							saveKageStudio(ksfPath, "<frame" + uintToString(j) + ">");
			...
			unsigned int i;
				...
				f = KageFramesManager::currentFrame;
					...
					for (j = 1; j <= l_fMax; ++j) {
						...
						if (j < 10) {
							l_pngSequencePath = l_pngPath + "0000" + uintToString(j) + ".png";
						} else if (j < 100) {
							l_pngSequencePath = l_pngPath + "000" + uintToString(j) + ".png";
						} else if (j < 1000) {
							l_pngSequencePath = l_pngPath + "00" + uintToString(j) + ".png";
						} else if (j < 10000) {
							l_pngSequencePath = l_pngPath + "0" + uintToString(j) + ".png";
						} else if (j < 100000) {
							l_pngSequencePath = l_pngPath + uintToString(j) + ".png";
						}
...
void Kage::ExportAVI_onClick() {
	...
	switch (result) {
		case Gtk::RESPONSE_OK:
			...
			string l_pngSequencePath;
			int l_len = strlen(l_pngPath.c_str()) - 4;
			if (strToLower(l_pngPath).substr(l_len, 4) == ".avi") {
				l_pngPath = l_pngPath.substr(0, l_len);
			}
...
void Kage::EntryFillAlpha_onEnter() {
	unsigned int l_uint = stringToUInt(m_EntryFillAlpha.get_text());
...
void Kage::EntryStrokeAlpha_onEnter() {
	unsigned int l_uint = stringToUInt(m_EntryStrokeAlpha.get_text());
...
void Kage::EntryStrokeThickness_onEnter() {
	unsigned int l_uint = stringToUInt(m_EntryStrokeThickness.get_text());
...
void Kage::EntryX_onEnter() {
	double l_dbl = stringToDouble(m_EntryX.get_text());
	
	m_EntryX.set_text(doubleToString(l_dbl));
	m_KageStage.updateShapeX(l_dbl);
}

void Kage::EntryY_onEnter() {
	double l_dbl = stringToDouble(m_EntryY.get_text());
	
	m_EntryY.set_text(doubleToString(l_dbl));
	m_KageStage.updateShapeY(l_dbl);
}

void Kage::EntryWidth_onEnter() {
	double l_dbl = stringToDouble(m_EntryWidth.get_text());
	
	m_EntryWidth.set_text(doubleToString(l_dbl));
	m_KageStage.updateShapeWidth(l_dbl);
}

void Kage::EntryHeight_onEnter() {
	double l_dbl = stringToDouble(m_EntryHeight.get_text());
...
//			(*toggleButtons.back()).add_pixlabel("shared/icons/" + label + "_tango.png",""); //find way to make the image centered
			(*toggleButtons.back()).add_pixlabel("shared/icons/" + label + ".png",""); //find way to make the image centered
...
bool Kage::saveKageStudio(string p_path, string p_msg) {
	...
	if (strToLower(p_path).substr(l_len, 4) != ".ksf") {
		p_path = p_path + ".ksf";
	}
...
string Kage::doubleToString(double p_src) {
	if (p_src == 0.0) {
		return "0.0";
	}
...
double Kage::stringToDouble(string strConvert) { 
	//TODO: validate string to be numerics
	return (double)atof(strConvert.c_str());
}
...
void Kage::updateSelectedShapeColor(bool p_doFill, bool p_doStroke) {
	...
	if (p_doFill == true) {
		...
		l_uint = stringToUInt(m_EntryFillAlpha.get_text());
		l_uint %= 256;
		m_EntryFillAlpha.set_text(uintToString(l_uint));
	...
	if (p_doStroke == true) {
		...
		l_uint = stringToUInt(m_EntryStrokeAlpha.get_text());
		l_uint %= 256;
		m_EntryStrokeAlpha.set_text(uintToString(l_uint));
		m_KageStage.fillColor.setA(l_uint);
		
		l_uint = stringToUInt(m_EntryStrokeThickness.get_text());
		KageStage::stroke.setThickness(l_uint);
	}
