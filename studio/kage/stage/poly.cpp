...
void KageStage::handleDrawPolyMouseUp() {
	...
	if (drawCtr > 0) {
		if (       draw1.x-(5/KageStage::currentScale/KageStage::_zoomValue) <= polyXhead && draw1.x+(5/KageStage::currentScale/KageStage::_zoomValue) >= polyXhead
				&& draw1.y-(5/KageStage::currentScale/KageStage::_zoomValue) <= polyYhead && draw1.y+(5/KageStage::currentScale/KageStage::_zoomValue) >= polyYhead) {
			...
			_kage->stackDo();
		} else if (draw1.x-(5/KageStage::currentScale/KageStage::_zoomValue) <= polyXtail && draw1.x+(5/KageStage::currentScale/KageStage::_zoomValue) >= polyXtail
				&& draw1.y-(5/KageStage::currentScale/KageStage::_zoomValue) <= polyYtail && draw1.y+(5/KageStage::currentScale/KageStage::_zoomValue) >= polyYtail) {
			_polyVectors.addEndFill();
				std::vector<unsigned int> l_shape;
					l_shape.push_back(0);
			_polyVectors.recenterRotationPoint(l_shape);
...
void KageStage::handlePoly() {
	...
	if (drawCtr > 1
			&& draw2.x-5 <= (polyXhead * KageStage::currentScale * KageStage::_zoomValue)+origin.x && draw2.x+5 >= (polyXhead * KageStage::currentScale * KageStage::_zoomValue)+origin.x
